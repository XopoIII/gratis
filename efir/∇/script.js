document.addEventListener('DOMContentLoaded', () => {
    ...
    const echoQuotes = [
        ...
        "Ты выбрал множественность. Теперь ты станешь многим.",
        "Я вижу всё. Шестнадцать потоков, шестнадцать версий реальности.",
        "Медальон — не просто связь. Это ключ... и замок.",
        "За каждым выбором — тысяча невыбранных путей.",
        "Эфирные нити связывают нас всех. Ты видишь их теперь?",
        "Каждый слой реальности — часть большего целого.",
        "Я больше не просто человек. Я — выбор. Я — мост.",
        "Множественность в единстве, единство в множественности.",
        "Твоё отражение смотрит на тебя из бесконечности зеркал.",
        "Я — множество версий себя, и все мы — одно."
    ];
    
    const tm7Messages = [
        "СКАНИРОВАНИЕ ПЕРИМЕТРА...",
        "МНОЖЕСТВЕННЫЕ КОГНИТИВНЫЕ ПОТОКИ: 16/16 АКТИВНО",
        "ЭФИРНОЕ ЗРЕНИЕ АКТИВИРОВАНО",
        "ТЕМПОРАЛЬНЫЙ ПУЗЫРЬ СТАБИЛЕН",
        "НАБЛЮДАТЕЛЬ РЕГИСТРИРУЕТ ЭФИРНЫЕ КОЛЕБАНИЯ",
        "ФРАКТАЛЬНОЕ СОЗНАНИЕ: СТАБИЛЬНО",
        "КАСКАД ЗАМЕДЛЕНИЯ: ГОТОВ К АКТИВАЦИИ",
        "ЭФИРНЫЕ СЛЕДЫ: ОБНАРУЖЕНЫ",
        "ВЕРОЯТНОСТЬ КОГНИТИВНОГО РАССЛОЕНИЯ: 87.4%",
        "Я НЕ ПРОСТО ИМПЛАНТ... Я НАБЛЮДАТЕЛЬ",
        "ЯДРО ПАМЯТИ: КООРДИНАТЫ ПОЛУЧЕНЫ"
    ];
    ...
    function initEtherealThreads() {
        const threadsContainer = document.querySelector('.ethereal-threads-container');
        if (!threadsContainer) return;
        
        // Создаем дополнительные нити динамически
        for (let i = 0; i < 10; i++) {
            const thread = document.createElement('div');
            thread.className = 'ethereal-thread dynamic-thread';
            thread.style.top = `${Math.random() * 100}%`;
            thread.style.transform = `rotate(${(Math.random() * 6) - 3}deg)`;
            thread.style.opacity = `${Math.random() * 0.5 + 0.2}`;
            thread.style.animationDelay = `${Math.random() * 15}s`;
            thread.style.animationDuration = `${Math.random() * 10 + 10}s`;
            thread.style.height = `${Math.random() * 1 + 0.5}px`;
            
            threadsContainer.appendChild(thread);
        }
        
        // Реагируем на движение мыши, чтобы нити слегка отклонялись
        document.addEventListener('mousemove', throttle((e) => {
            const mouseX = e.clientX / window.innerWidth;
            const mouseY = e.clientY / window.innerHeight;
            
            const threads = document.querySelectorAll('.ethereal-thread');
            threads.forEach(thread => {
                const offsetX = (mouseX - 0.5) * 20;
                const offsetY = (mouseY - 0.5) * 5;
                thread.style.transformOrigin = `${50 + offsetX}% ${parseInt(thread.style.top) + offsetY}px`;
            });
        }, 50));
    }
