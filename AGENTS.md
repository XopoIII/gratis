...
- Sections: each area is a folder with its own `index.html`, `style.css`, `script.js`, and assets (e.g., `broken/`, `navmor/`, `stalker/`).
- Assets: images live next to the page that uses them (e.g., `navmor/*.jpg`). Keep filenames lowercase, hyphen-separated.
